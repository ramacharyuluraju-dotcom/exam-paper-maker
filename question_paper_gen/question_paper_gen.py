...
with t_edit:
    ...
    # --- HEADER DETAILS (Fixed: Removed st.form so Save Draft works) ---
    with st.expander("📝 Header Details", expanded=True):
        c1, c2, c3, c4 = st.columns(4)
        
        # 1. ACADEMIC YEAR
        st.session_state.exam_details['acadYear'] = c1.text_input(
            "Academic Year", 
            st.session_state.exam_details.get('acadYear', '2024-2025'), 
            disabled=read_only
        )
        ...
        st.session_state.exam_details['department'] = c2.selectbox(
            "Department", DEPTS, index=idx_dept, disabled=read_only
        )
        
        # 3. SEMESTER
        curr_sem = st.session_state.exam_details.get('semester')
        idx_sem = SEMESTERS.index(curr_sem) if curr_sem in SEMESTERS else 0
        st.session_state.exam_details['semester'] = c3.selectbox(
            "Semester", SEMESTERS, index=idx_sem, disabled=read_only
        )
        ...
        st.session_state.exam_details['examType'] = c4.selectbox(
            "Exam Type", EXAM_TYPES, index=idx_type, disabled=read_only
        )
        
        c1, c2, c3 = st.columns(3)
        
        # 5. DATE (Handle String/Date conversion safely)
        try:
            date_val = datetime.datetime.strptime(
                str(st.session_state.exam_details.get('examDate', datetime.date.today())), 
                "%Y-%m-%d"
            ).date()
        except:
            date_val = datetime.date.today()
            
        new_date = c1.date_input("Exam Date", value=date_val, disabled=read_only)
        st.session_state.exam_details['examDate'] = str(new_date)
        
        # 6. COURSE CODE
        st.session_state.exam_details['courseCode'] = c2.text_input(
            "Course Code", 
            st.session_state.exam_details.get('courseCode', ''), 
            disabled=read_only
        )
        
        # 7. COURSE NAME
        st.session_state.exam_details['courseName'] = c3.text_input(
            "Course Name", 
            st.session_state.exam_details.get('courseName', ''), 
            disabled=read_only
        )
            ...
            if section.get('isNote'):
                ...
                for j, q in enumerate(section['questions']):
                    ...
                    if q['text'].upper() != 'OR':
                        ...
                        total_marks += q['marks']
                        q['co'] = m2.selectbox("CO", COS_LIST, key=f"co_{q['id']}", index=COS_LIST.index(q['co']) if q['co'] in COS_LIST else 0, disabled=read_only)
                        q['level'] = m3.selectbox("L", BLOOMS_LEVELS, key=f"lv_{q['id']}", index=BLOOMS_LEVELS.index(q['level']) if q['level'] in BLOOMS_LEVELS else 0, disabled=read_only)
        ...
        if c1.button("💾 Save Draft") and d.get('courseCode'):
            if db:
                try:
                    ...
                    # (Variables are already updated in session_state via the inputs above)
                    data = {
                    ...
                    st.success(f"Saved Draft: {current_id}")
                except Exception as e:
